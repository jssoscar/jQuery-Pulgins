...
jQuery.timer = function(options) {
    ...
    var config = (function() {
        ...
        defaultCfg.startTime = calTime(defaultCfg.startTime);
        
        // Deal with the end time
        defaultCfg.endTime = calTime(defaultCfg.endTime);
        
        // Deal with the server time
        if(!defaultCfg.serverTime || isNaN(defaultCfg.serverTime)){
        	defaultCfg.serverTime = 0;
        }
        return defaultCfg;
